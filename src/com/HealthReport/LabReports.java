...
public class LabReports {
    ...
    public void insertData(String type, String status, String info) throws Exception {
        Sql.Execute(!hasData(id)?"INSERT INTO lab_reports (ID,Type,Status,Info) VALUES ('" + id + "','" + type + "','" + status + "','" + info + "')":"UPDATE lab_reports SET Type='"+type+"',Status='"+status+"',Info='"+info+"' WHERE ID='"+id+"'", CONN);
    }

    public static boolean hasData(String id){
        return Sql.getValue("SELECT ID FROM lab_reports WHERE ID='"+id+"'", CONN)!=null;
    }
