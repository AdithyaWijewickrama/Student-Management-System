...
//        ID++;
    ...
    public static void main(String[] args) {
        for (int g = 1; g <= 13; g++) {
            for (String sg : new String[]{"A", "B", "C", "D"}) {
                DeleteStudent ds = new DeleteStudent(PERMENATLY_DELETE);
                List<Object> idss = Sql.getColumn("SELECT \"" + ID_NO + "\" FROM student where Grade='" + g + "' and \"Sub grade\"='" + sg + "'", 0, CONN);
                for (int i = 0; i < idss.size()-10; i++) {
                    ds.ids.add(idss.get(i));
                }
                ds.deleteAccounts();
            }
        }
